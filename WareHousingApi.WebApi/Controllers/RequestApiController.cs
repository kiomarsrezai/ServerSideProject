...
using WareHousingApi.Entities.Models.Dto.Request;

namespace WareHousingApi.Areas.Api.Controllers.v1
{
    ...
    public class RequestApiController : ControllerBase
    {
        ...
        [Route("RequestCreate")]
        [HttpPost]
        public async Task<ApiResult<List<RequestAfterInsertViewModel>>> RequestCreate([FromBody] RequestInsertViewModel viewModel)
        {
            ...
            using (SqlConnection sqlconnect = new SqlConnection(_config.GetConnectionString("WareHousingApiConnectionString")))
            {
                using (SqlCommand sqlCommand = new SqlCommand("SP010_Request_Insert", sqlconnect))
                {
                    ...
                    while (dataReader.Read())
                    {
                        ...
                        request.DateShamsi = Common.DateTimeExtensions.ConvertMiladiToShamsi(DateTime.Parse(dataReader["Date"].ToString()), "yyyy/MM/dd");
